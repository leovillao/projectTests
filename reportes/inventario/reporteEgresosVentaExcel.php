...
if (isset($_POST['cliente']) && !empty($_POST['cliente'])) { // proveedores
    $where .= 'and prid = ' . PersonData::getByceRuc($_POST['cliente'])->ceid;
}
...
//    $hoja->setCellValueByColumnAndRow(7, 3, 'Saldo');
        ...
        foreach ($compras as $venta) {
            ...
            if ($venta->ceid != null) {
                $hoja->setCellValueByColumnAndRow(5, $i, PersonData::getById($venta->ceid)->cename);
            } else {
...
// TITULO DE LA PAGINA
        ...
        $hoja->setCellValueByColumnAndRow(7, 1, "Usuario : " . UserData::getById($_SESSION['user_id'])->name);
        /** ======================================*/
        $hoja->setCellValueByColumnAndRow(1, 3, 'Producto');
        $hoja->setCellValueByColumnAndRow(2, 3, 'Cantidad');
        $hoja->setCellValueByColumnAndRow(3, 3, 'Factura');
        $hoja->setCellValueByColumnAndRow(4, 3, 'Unidad');
        $hoja->setCellValueByColumnAndRow(5, 3, 'Costo');
        $hoja->setCellValueByColumnAndRow(6, 3, 'Total');
//    $hoja->setCellValueByColumnAndRow(7, 3, 'Saldo');
        $i = 4;
        $o = 0;
        foreach ($compras as $venta) {
            ...
            $hoja->setCellValueByColumnAndRow(1, $i, ProductData::getById($venta->itid)->itname);
            $hoja->setCellValueByColumnAndRow(2, $i, FData::formatoNumeroReportsInventario($venta->odcandig));
            $hoja->setCellValueByColumnAndRow(3, $i, (!is_null($venta->fi_id)) ? FilesData::getByIdOne($venta->fi_id)->fi_docum : 'PENDIENTE');
            $hoja->setCellValueByColumnAndRow(4, $i, UnitData::getById($venta->unid_dig)->undescrip);
            $hoja->setCellValueByColumnAndRow(5, $i, FData::formatoNumeroReportsInventario($venta->odcostoudig));
            $hoja->setCellValueByColumnAndRow(6, $i, FData::formatoNumeroReportsInventario($venta->odcostotot));
