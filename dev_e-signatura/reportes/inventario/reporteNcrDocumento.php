...
foreach ($ventas as $venta) {
    if ($venta->fi_tipo == '04') {
        ...
        $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? FData::formatoNumeroReportes($venta->fi_subtotal - $venta->fi_desc + $venta->fi_iva ) : 0.00, 'T,B,R', 1, 'R', 0, 0);

        $pdf->SetFont('Arial', '', 7); // titulos

        $totalST = $totalST + $venta->fi_subtotal;

        $totalNeto = $totalNeto + ($venta->fi_subtotal - $venta->fi_desc + $venta->fi_iva);
        ...
        $neto = $neto + ($venta->fi_subtotal - $venta->fi_desc + $venta->fi_iva);
    }
