...
//    echo json_encode($ventas);
  ...
  $spread->getActiveSheet()->getStyle('A3:L3')->getFont()->setBold(true)->setSize(10);
...
//    $hoja->setCellValueByColumnAndRow(1, 5, "Un valor en 1, 1");
  ...
  $hoja->setCellValueByColumnAndRow(6, 2, ($_POST['vendedor'] == 0)?" Vendedor : Todos":"Vendedor : " .VendedorData::getById($_POST['vendedor'])->venombre );
...
//  Fecha Doc Producto Unidad Cantidad Precio Subtotal Iva Total
  $hoja->setCellValueByColumnAndRow(1, 3, 'Vendedor');
  $hoja->setCellValueByColumnAndRow(2, 3, 'Fecha');
  $hoja->setCellValueByColumnAndRow(3, 3, 'Tipo');
  ...
  $hoja->setCellValueByColumnAndRow(7, 3, 'Cantidad');
  $hoja->setCellValueByColumnAndRow(8, 3, 'Precio');
  $hoja->setCellValueByColumnAndRow(9, 3, 'Subtotal');
  $hoja->setCellValueByColumnAndRow(10, 3, 'Iva');
  $hoja->setCellValueByColumnAndRow(11, 3, 'Total');
  $i = 4;
  foreach ($ventas as $venta) {

    /*"fecha" => $venta->fi_fechadoc,
            "tipoDoc" => $tipoDoc,
            "documento" => $venta->fi_docum,
            "producto" => ucwords(strtolower($venta->itname)),
            "unidad" => $venta->undescrip,
            "cantidad" => $venta->odcandig,
            "precio" => $venta->fi_iva,
            "subtotal" => $venta->odsubtotal,
            "iva" => $venta->odiva,
            "neto" => $venta->fi_neto,*/

    $hoja->setCellValueByColumnAndRow(1, $i, VendedorData::getById($venta->veid)->venombre);
    $hoja->setCellValueByColumnAndRow(2, $i, $venta->fi_fechadoc);
    $hoja->setCellValueByColumnAndRow(3, $i, Validacion::tipoDocumento($venta->fi_tipo));
    $hoja->setCellValueByColumnAndRow(4, $i, $venta->fi_docum);
    $hoja->setCellValueByColumnAndRow(5, $i, ucwords(strtolower($venta->itname)));
    $hoja->setCellValueByColumnAndRow(6, $i, $venta->undescrip);
    $hoja->setCellValueByColumnAndRow(7, $i, $venta->odcandig);
    $hoja->setCellValueByColumnAndRow(8, $i, $venta->odpvp);
    $hoja->setCellValueByColumnAndRow(9, $i, $venta->odsubtotal);
    $hoja->setCellValueByColumnAndRow(10, $i, $venta->odiva);
    $hoja->setCellValueByColumnAndRow(11, $i, $venta->odtotal);
  ...
  $hoja->setCellValueByColumnAndRow(9, $i, $subt);
  $hoja->setCellValueByColumnAndRow(10, $i, $iva);
  $hoja->setCellValueByColumnAndRow(11, $i, $neto);
  $fileName = "Informe_de_Ventas_ProductosVendedor_SMARTTAG_BI.xlsx";
  # Crear un "escritor"
