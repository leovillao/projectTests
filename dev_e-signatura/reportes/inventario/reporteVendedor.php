<?php
// REPORTE DE VENTA POR VENDEDOR / PRODUCTO
...
foreach ($ventas as $venta) {
    ...
    if ($venta->veid != $vendedor) {
        if ($venta->fi_ptoemi != $emision) {
            ...
            $pdf->Cell(189, $alto, utf8_decode("VENDEDOR " . VendedorData::getById($venta->veid)->venombre), 'L,T,B,R', 1, 'L', 0, 0);
    ...
    $pdf->Cell(55, $alto, substr(utf8_decode(ucwords(strtolower($venta->itname))) . $anulado, 0, 46), 'T,B,R', 0, 'L', 0, 0);
    $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? $venta->undescrip : "", 'T,B,R', 0, 'C', 0, 0);
    $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? FData::formatoNumeroReportes($venta->odcandig) : 0.00, 'T,B,R', 0, 'R', 0, 0);
    $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? FData::formatoNumeroReportes($venta->odpvp) : 0.00, 'T,B,R', 0, 'R', 0, 0);
    $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? FData::formatoNumeroReportes($venta->odsubtotal) : 0.00, 'T,B,R', 0, 'R', 0, 0);
    $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? FData::formatoNumeroReportes($venta->odiva) : 0.00, 'T,B,R', 0, 'R', 0, 0);
    $pdf->Cell(16, $alto, ($venta->fi_estado != 3) ? FData::formatoNumeroReportes($venta->odtotal) : 0.00, 'T,B,R', 1, 'R', 0, 0);

    $pdf->SetFont('Arial', '', 7); // titulos

    /*SUBTOTAL VENDEDORES*/
    $totCantV = $totCantV + $venta->odcandig;
    $totPvpV = $totPvpV + $venta->odpvp;
    $totSubtV = $totSubtV + $venta->odsubtotal;
    $totIvaV = $totIvaV + $venta->odiva;
    $totNetoV = $totNetoV + $venta->odtotal;

    /*SUBTOTALES SUCURSAL*/
    $totalCant = $totalCant + $venta->odcandig;
    $totalPvp = $totalPvp + $venta->odpvp;
    $totalSubt = $totalSubt + $venta->odsubtotal;
    $totalIva = $totalIva + $venta->odiva;
    $totalNeto = $totalNeto + $venta->odtotal;

    /*TOTALES SUCURSAL*/
    $tcant = $tcant + $venta->odcandig;
    $pvp = $pvp + $venta->odpvp;
    $subt = $subt + $venta->odsubtotal;
    $tiva = $tiva + $venta->odiva;
...
if ($totNetoV >= 0) {
    ...
    $pdf->Cell(16, $alto, "", 'T,B,R', 0, 'R', 1, 0);
    $pdf->Cell(16, $alto, "", 'T,B,R', 0, 'R', 1, 0);
    $pdf->Cell(16, $alto, FData::formatoNumeroReportes($totSubtV), 'T,B,R', 0, 'R', 1, 0);
    $pdf->Cell(16, $alto, FData::formatoNumeroReportes($totIvaV), 'T,B,R', 0, 'R', 1, 0);
    $pdf->Cell(16, $alto, FData::formatoNumeroReportes($totNetoV), 'T,B,R', 1, 'R', 1, 0);
}
if ($totalNeto >= 0) {
    $pdf->SetFillColor(231, 229, 229);
    $pdf->Cell(23, $alto, '', 'L,T,B', 0, 'L', 1, 0);
