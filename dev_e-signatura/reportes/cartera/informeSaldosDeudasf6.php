...
foreach ($cobros as $cobro) {
    ...
    if (empty($nameSucursal)) {
        $nameSucursal = $cobro->venombre;
    }
    if (empty($tdname)) {
        $tdname = $cobro->tdnombre;
    }
    if ($cobro->veid != $idCobro) {
        if ($totalFP > 0) {
        ...
        $pdf->Cell(189, $alto, utf8_decode("VENDEDOR : " . $nameSucursal), 'L,T,B,R', 1, 'L', 0, 0);
        $pdf->Cell(189, $alto, utf8_decode("TIPO DOCUMENTO : ") . utf8_decode($tdname), 'L,T,B,R', 1, 'L', 0, 0);
