...
//    $pdf->SetFont('Arial', '', 10); // titulos
    ...
    $pdf->Cell(190, 5, 'Resumido', 0, 1, 'C', 0, 0);
    ...
    foreach ($compensaciones as $venta) {
        if ($documento != $venta->crid) {
            ...
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(80, $altoTitulo, $venta->crcomenta, 0, 1, 'L', 0, 0);
            ...
            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(20, $altoTitulo, 'VALOR : ', 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(90, $altoTitulo, "$" . $venta->crtotal, 0, 1, 'L', 0, 0);
            $pdf->Ln(2);
            $pdf->Cell(190, 1, '', "B", 1, 'L', 0, 0);
            $pdf->Ln(2);
            $pdf->SetFont('Arial', '', 7); // titulos
...
//            $totalesGeneralesDeuda += $ttotalDeuda;
            $totalesGeneralesAnt += $venta->crtotal;
        }
    }
    $pdf->Cell(25, $alto, '', 'L,T,B', 0, 'L', 0, 0);
    $pdf->Cell(30, $alto, '', 'T,B', 0, 'L', 0, 0);
    ...
    $pdf->SetFont('Arial', '', 7); // titulos
    $pdf->Cell(26, $alto, FData::formatoNumeroReportes($totalesGeneralesAnt), 'L,T,B,R', 0, 'R', 0, 0);
    $pdf->Cell(31, $alto, "", 'L,T,B,R', 1, 'R', 0, 0);
    $pdf->Output();
} else {
    echo '<script>
