...
// Cabecera de página
    public function Header()
    {
        ...
        if (UserData::getById($_SESSION['user_id'])->is_admin == 1) {
            $this->Ln(5);
            $this->Cell(98, 5, 'VENDEDOR : TODOS', 0, 1, 'L', 0, 1);
            $this->Cell(90, 4, 'Desde : ' . $GLOBALS['desde'] . ' / Hasta : ' . $GLOBALS['hasta'], 0, 0, 'L', 0, 1);
            $this->Ln(3);
            $this->SetFont('Arial', 'B', 9);
            $this->Cell(190, 5, 'DETALLADO POR FORMAS', 0, 1, 'C', 0, 1);
            $this->Cell(18, 5, 'Fecha', 'T,B,L,R', 0, 'C', 0, 1);
            $this->Cell(15, 5, 'Idcobro', 'T,B,L,R', 0, 'C', 0, 1);
            $this->Cell(87, 5, 'Cliente', 'T,B,L,R', 0, 'C', 0, 1);
            $this->Cell(30, 5, 'Banco', 'T,B,L,R', 0, 'C', 0, 1);
            $this->Cell(15, 5, '#Doc ', 'T,B,L,R', 0, 'C', 0, 1);
            $this->Cell(25, 5, 'Valor', 'T,B,L,R', 1, 'C', 0, 1);
        }else{
            $this->SetFont('Arial', 'B', 9);
            $this->Ln(5);
            $this->Cell(98, 5, 'VENDEDOR : ' . UserData::getById($_SESSION['user_id'])->name, 0, 1, 'L', 0, 1);
            $this->Cell(90, 4, 'Desde : ' . $GLOBALS['desde'] . ' / Hasta : ' . $GLOBALS['hasta'], 0, 0, 'L', 0, 1);
            $this->Ln(3);
            $this->SetFont('Arial', 'B', 9);
            $this->Cell(190, 5, 'DETALLADO POR FORMAS', 0, 1, 'C', 0, 1);
            $this->Cell(30, 5, 'Idcobro', 'T,B,L,R', 0, 'L', 0, 1);
            $this->Cell(53, 5, 'Cliente', 'T,B,L,R', 0, 'L', 0, 1);
            $this->Cell(42, 5, 'Banco', 'T,B,L,R', 0, 'L', 0, 1);
            $this->Cell(22, 5, '#Doc', 'T,B,L,R', 0, 'L', 0, 1);
            $this->Cell(18, 5, 'Fecha', 'T,B,L,R', 0, 'L', 0, 1);
            $this->Cell(25, 5, 'Valor', 'T,B,L,R', 1, 'L', 0, 1);
        }
    }
}
...
if (UserData::getById($_SESSION['user_id'])->is_admin == 1) {
    ...
    $detCobros = ReporteCobros::getDetInfoCobros($_POST);
//    var_dump($detCobros);
