...
// Cabecera de página
    public function Header()
    {
        ...
        $this->Ln(5);
        $this->Cell(190, 7, $GLOBALS['titulo'] . " ANULADAS ", 0, 1, 'C', 0, 0);
    }
}
...
$compensaciones = CrucecabData::getAllFechasCabAnulados($where);
...
//    $pdf->SetFont('Arial', '', 10); // titulos
    ...
    foreach ($compensaciones as $venta) {
        if ($documento != $venta->crid) {
            ...
            $pdf->Cell(30, $altoTitulo, 'CLIENTE : ', 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(80, $altoTitulo, strtoupper(utf8_decode($venta->cename)), 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(43, $altoTitulo, 'USUARIO CREACION : ', 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(40, $altoTitulo, strtoupper(utf8_decode(UserData::getById($venta->user_id)->name)), 0, 1, 'L', 0, 0);

            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(30, $altoTitulo, utf8_decode('COMPENSACIÓN : '), 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(80, $altoTitulo, $venta->crid, 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(30, $altoTitulo, utf8_decode('FECHA CREACION : '), 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(40, $altoTitulo, $venta->crcreate_at, 0, 1, 'R', 0, 0);



            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(30, $altoTitulo, 'COMENTARIO : ', 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(80, $altoTitulo, $venta->crcomenta, 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(30, $altoTitulo, utf8_decode('FECHA ANULACION : '), 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(40, $altoTitulo, $venta->cranula_at, 0, 1, 'R', 0, 0);


            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(20, $altoTitulo, 'FECHA : ', 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(90, $altoTitulo, $venta->crfecha, 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', 'B', 8); // titulos
            $pdf->Cell(43, $altoTitulo, 'USUARIO ANULACION : ', 0, 0, 'L', 0, 0);
            $pdf->SetFont('Arial', '', 8); // titulos
            $pdf->Cell(40, $altoTitulo, strtoupper(utf8_decode(UserData::getById($venta->user_anula)->name)), 0, 1, 'L', 0, 0);
