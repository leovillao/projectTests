...
//        $proccError = true;
                    if ($proccError == true) {
                        ...
                        $resultEnvio = self::loadFileRsm($claveautorizacion, $nameDocum, $idFiles, $resultFileXml);
                ...
                $upd = $updateClave->updateStateAuto();
                $updateState = new FilesData();
    ...
    public static function procesarDocRet($nameDocum, $idFiles) {
        $pathXml = EmpresasData::getEmpresaData();
        $ch = curl_init($pathXml->em_wsdl);
        $fxml = "xml/" . $_SESSION['ruc'] . "/sinfirma/Ret" . $nameDocum . ".xml";
        /** ARCHIVO CONSTRUIDO */
        $xml = self::makeCurlFile($fxml);
        $resultXml = '';
        /** ===============================
...
//                        unlink("xml/" . $pathXml->em_ruc . "/sinfirma/Ret" . $nameDocum . ".xml");
                        $updateState->CerrarTransaccion($conx2);
                        $resultEnvio = self::loadFileRsmRet($idFiles, $resXml,$nameDocum);
                        $resultAuto = "1-Documento #" . $nameDocum . " autorizado con exito" ;
                    }
                }
            } else {
                ...
                $updateClave->fi_fecauto = ' null';
                $updateClave->fi_id = $idFiles;
                $updateClave->fi_logauto = '"'.$arrayXml->mensaje .'"';
                $upd = $updateClave->updateStateAutoR();
...
//            $ch = curl_init("http://pyme.e-piramide.net/downxml/saveXml.php");
    ...
    public static function procesarDocN($nameDocum, $idFiles) {
        ...
        if ($arrayXml->estado == 'AUTORIZADO') {
            ...
            $resAuto = file_get_contents("http://pyme.e-piramide.net/downxml/descarga.php?clave=" . $claveAccesoString);
...
//            $resultEnvio = self::loadFileRsm($r[1], $nameDocum, $idFiles);
    ...
    public static function loadFileRsm($claveAcceso, $numFact, $idFiles,$archivo) {
...
//    } else {
//      array_push($arrau, "0-Error al enviar por correo");
//      array_push($arrau, "");
//    }
        if ($error == false) {
...
//      array_push($arrau, json_encode($archivo));
    ...
    public static function loadFileRsmRet($idFiles,$archivo,$numFact) {
        /**====== CON ESTA FUNCION SE GRABA EL ARCHIVO XML EN LA TABLA RSMFILE SI EL DOCUMENTO FUE AUTORIZADO =======*/
        $load = '';
        $error = false;
        $msjError = '';
...
//    if (substr($estadoSave, 0, 1) != 0) {
        /** ======== FUNCION PARA EL PROCESO DE ENVIO DEL DOCUMENTO POR CORREO ELECTRONICO ========= */
        $msjCorreos = PdfData::GeneraPdfRetencion($idFiles);
        if (substr($msjCorreos, 0, 1) == 1) {
            // SE VALIDA SI EL ENVIO DE CORREO FUE CORRECTO
            $upd_Send = new FilesData();
            $upd_Send->fi_stasend = 1;
            ...
            unlink('xml/' . $_SESSION['ruc'] . '/pdf/Ret' . $numFact . '.pdf');
            unlink('xml/' . $_SESSION['ruc'] . '/autorizados/Ret' . $numFact . '.xml');
        } else {
            $error = true;
            $msjError = "0-Documento no se envio por correo";
            unlink('xml/' . $_SESSION['ruc'] . '/pdf/Ret' . $numFact . '.pdf');
