$(function () {
    ...
    $("#buscar-productos").keyup(function () {
        ...
        if ($(this).val() !== '') {
            ...
            $.each(productosobj, function (i, item) {
                if (inc <= top) {
                    tableBodyProduct += '<tr id="row-' + inc + '" class="remove-class"><td>' + inc + '</td><td>' + item.itcodigo + '<input type="hidden" value="' + item.iniva + '"></td><td>' + item.name + '<input type="hidden" value="' + item.codigo + '"></td><td>' + item.unidad + '</td><td><buttom class="btn-loadBodega btn btn-listar btn-xs"><i class="glyphicon glyphicon-inbox"></i></buttom><buttom class="btn btn-imagen btn-xs" imagen="' + item.imagen + '"><i class="glyphicon glyphicon-picture"></i></buttom></td></tr>'
                }
                inc++
            })
        } else {
    ...
    $(document).on("click", "#btn-updateCotizacion", function (e) {
        ...
        } else if ($("#vendedorid").val() == "") {
            Swal.fire({
                icon: 'error',
                title: 'Debe seleccionar vendedor',
            })
        } else {
            procesarCompra(tipoProcesso)
        }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(datos, function (i, item) {
                        ...
                        tableBodyProduct += '<tr  id="row-' + t + '" class="remove-class"><td>' + t + '</td><td>' + item.itcodigo + '<input type="hidden" value="' + item.iniva + '"></td><td>' + item.name + '<input type="hidden" value="' + item.codigo + '"></td><td>' + item.unidad + '</td><td>' + item.stock + '</td><td><button class="btn-loadBodega btn btn-listar btn-sm"><i class="glyphicon glyphicon-inbox"></i></button></td></tr>'
                        t++
                    })
                    $("#table-tbody-products").html(tableBodyProduct)
                }
    ...
    * */
    ...
    // $(document).on('click', '.btn-loadBodega', function (e) {
    //     let row = $(this)
    ...
    $(document).on('click', '.btn-loadBodega', function () {
        ...
        // selectProducto($(this), "")
        let fecha = $("#fechaDoc").val()
        let iniva = $(this).closest('tr').find('td').eq(1).find('input[type="hidden"]').val()
        let id = $(this).closest('tr').find('td').eq(2).find('input[type="hidden"]').val()
        let codigo = $(this).closest('tr').find('td').eq(1).text()
        let producto = $(this).closest('tr').find('td').eq(2).text()
        let unidad = $(this).closest('tr').find('td').eq(3).text()
        selectProducto(codigo, producto, iniva, id, "")
        modalConfirmacion(codigo, producto, unidad)
    })  /* ==== Click sobre tbody PRODUCTOS / CARGA DE CANTIDADES POR BODEGA ===== */

    function modalConfirmacion(codigo, producto, unidad) {
        $("#modalConfirmacion .modal-header").css("background-color", "#C2CF2D")
        $('#modalConfirmacion').modal('toggle').on('shown.bs.modal', function () {
            $("#modalConfirmacion .modal-title").text(codigo + " - " + producto + " - " + unidad)
            $("#modalConfirmacion #confirmarAceptar").focus()
        })
        loadSaldoBodegasProductos(codigo)
        $("#modalProducto").modal('toggle')
    }

    function loadSaldoBodegasProductos(codigo) {
        $("#content-table").html('')
        $.ajax({
            url: './?action=loadStockActualizadoCotizacion',
            type: 'POST',
            data: {"codigo": codigo, "bodega": $("#bodega").val()},
            success: function (respond) {
                console.log(respond)
                let res = JSON.parse(respond)
                let option = ""
                for (let index = 0; index < res.det.length; index++) {
                    option += "<table class='table table-bordered table-condensed'>"
                    option += '<tr><th style="background:#D6D5CA">Bodega</th><th>Cantidad</th></tr>'
                    res.det[index].forEach(element => {
                        option += '<tr><td style="background:#D6D5CA">' + element.bodega + '</td><td>' + element.saldo + '</td></tr>'
                    });
                    option += "</table>"
                }
                $("#content-table").html(option)
            }
        })
    }
    ...
    *    CARGA LA INFORMACION DE LAS UNIDADES DEL PRODUCTO SELECCIONADO DESDE LA VENTANA DE AYUDA DE PRODUCTOS */
    function loadUnit() {
        ...
        $.ajax({
            ...
            success: function (e) {
                ...
                $("#cantidad").focus()
                loadPriceFromListFDO(1)
            }
        })
    }

    function loadPriceFromListFDO(factor) {
        ...
        if (unidad != '') {
            ...
            $.ajax({
                type: "POST",
                url: "./?action=cotizacion_getPrecioFDO",
                data: {
                    "unidad": unidad,
                    "formapago": 1,
                    "producto": itcodigo,
                ...
                success: function (e) {
                    ...
                    if (dat.definicion == 1) {
                        ...
                        if (dat.precio.stock <= 0)  {
                            Swal.fire({
                                icon: 'error',
                                title: 'Oops...',
                                html: '<h3>Sin Unidades Disponible!</h3>',
    ...
    $(document).on('click', '#addLineaProducto', function (e) {
        ...
        let precio = $("#precion").val()
        let valida = JSON.parse(validaStockProduct())
        console.log(valida)
        if (valida.validacion == 1) {
            if (parseFloat(valida.stock) < parseFloat(cant)) {
                Swal.fire({
                    title: 'No disponible ',
                    text: "La cantidad requerida : " + cant + " , Stock disponible : " + valida.stock,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
    ...
    function loadLineaProduct() {
        ...
        if (producto != '') {
            ...
            option += '<td>' + codigo + '<input type="hidden" name="iniva" value="' + iniva + '"></td>' +
                ...
                /*7*/'<td class="alignTextInputNumber"><input type="hidden" value="' + unidad + '">' + (cantidad * nw_precio).toFixed(2) + '</td>' +
    ...
    $(document).on('click', '#table-cotizaciones tbody tr', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // $("#clienteIdCotizacion").val(r.cabecera.peid)
