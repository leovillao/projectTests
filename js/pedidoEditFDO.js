if (document.getElementById('pedidoEditFDO')) {
    $(document).ready(function () {
            ...
            $(document).on("click", "#btn-actualizar-pedidos", function (e) {
                e.preventDefault()
            ...
            function procesarActualizacion() {
                ...
                $("#tbody-saleProducts tr").each(function () {
                    ...
                    let can = row.find('td').eq(2).find('input[type="number"]').attr('cantidad')
                    ...
                    if (can != cantidad) {
                        ...
                        let id = row.find('td').eq(2).find('input[type="number"]').attr('id')
                ...
                formData.append('numPedido', $("#numPedido").val())
                formData.append('comentario', $("#observacion").val())
            ...
            * */
            ...
            const np = $("#numPedido").val()
            loadPedidosData(np)

            function loadPedidosData(numPedido) {
                $("#tbody-saleProducts").html('')
                $.post("./?action=loadPedforEdit", {pedido: numPedido})
                    .done(function (data) {
                        const datos = JSON.parse(data)
                        const r = datos.data.filter(dato => dato.saldo == 1) // array con productos sin novedad de STOCK
                        ...
                        $.each(r, function (i, item) {
                            htmlTable += "<tr>"
                            htmlTable += "<td><input type='hidden' name='iniva' value='" + item.iniva + "'>" + item.codigo + "</td><td><input name='funcion' type='hidden' value='0'>" + item.producto + "</td><td><input name='costo' type='hidden' value='0'><input class='form-control cantPedDet' id='" + item.id + "' size='4' type='number' value='" + item.cantidad + "' cantidad='" + item.cantidad + "'></td><td><input type='hidden' name='unidad' value='" + item.unidadDigitada + "'>" + item.unidadDigitadaName + "</td><td><input name='unidad' type='hidden' value='0'>" + item.precio + "</td><td>" + parseFloat(item.cantidad * item.precio).toFixed(5) + "</td><td>" + item.total + "</td><td></td>"
                            htmlTable += "</tr>"
            ...
            $(document).on('blur', '.cantPedDet', function (e) {
                e.preventDefault()
                let id = $(this).attr('id')
                let valor = $(this).val()
                $(this).val(parseFloat(valor).toFixed(4))
