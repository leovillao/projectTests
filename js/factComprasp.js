if (document.getElementById('factCompras')) {
    $(function () {

            confValorIngreso()

            function confValorIngreso() {
                let confDato = $("#confValorIngreso").val()
                console.log(confDato)
                /**1 - ingresa costo unitario
                 2 - ingresa el costo total*/
                if (confDato == 1) {
                    $("#total").prop("readonly", true);
                    $("#precion").prop("readonly", false);
                } else {
                    $("#total").prop("readonly", false);
                    $("#precion").prop("readonly", true);
                }
            }
            ...
            $("#cliente-cod").blur(function () {
                if ($(this).val() != "") {
                    $.ajax({
                        url: './?action=in_validaCodProveedor',
                        type: "POST",
                        ...
                        success: function (respond) {
                            ...
                            if (r != "") {
                                $("#cliente-cod").val(r.codigo)
                                $("#cliente-text").val(r.name)

                            } else {
                                Swal.fire({
                                    // position: 'top-end',
                                    icon: 'error',
                                    title: "Proveedor no exite en la ficha de proveedores.",
            ...
            $("#numerofact").blur(function () {
                let documento = $("#tipoDocumento")
                let proveedor = $("#cliente-cod").val()
                let emision = $("#ptoemision").val()
                let factura = $("#numerofact").val()
                let cod = $('option:selected', documento).attr('cod');
                $.ajax({
                    url: './?action=in_validaNumeroDocumento',
                    type: "POST",
                    data: {"cod": cod, "emision": emision, "factura": factura, "proveedor": proveedor},
                    success: function (respond) {
                        let r = JSON.parse(respond)
                        if (r != "") {
                            Swal.fire({
            ...
            $("#autorizacion").mask('0000000000000000000000000000000000000000000000000');
            $("#ptoemision").mask('000000');

            $(document).on('click', '#cargarOrden', function () {
                $("#modalOrdenes").modal('toggle')
                getOrdenesDespacho()
            })

            $(document).on("click", "#btn-retencion", function () {
                $("#modalRetencion").modal('toggle')
            })

            function getOrdenesDespacho() {
                $("#table-ordenes").DataTable({
                    "destroy": true,
                    "keys": true,
                    "ajax": {
                        "method": "POST",
                        "url": "./?action=getOrdenes",
                        "data": {"option": 1}
                    },
                    "columns": [
                        {"data": "id"},
            ...
            $(document).on('click', '#table-ordenes tbody tr', function () {
                let id = $(this).closest('tr').find('td').eq(0).text()
                let codigo = $(this).closest('tr').find('td').eq(1).text()
                let proveedor = $(this).closest('tr').find('td').eq(2).text()
                $("#cliente-cod").val(codigo)
                $("#ordenCompra").val(id)
                $("#cliente-text").val(proveedor)
                $("#ordenCompraNumero").val(id).prop('readonly', true)
                loadDataOrdenes(id)
            })

            function loadDataOrdenes(idOrden) {
                $.ajax({
                    url: './?action=loadDetalleOrden',
                    type: "POST",
                    data: {"option": 1, "idorden": idOrden},
                    success: function (respond) {
                        ...
                        $.each(res.data, function (i, item) {
                            ...
                            tr += '<tr>\n' +
                                '<td>' + item.itcodigo + '<input type="hidden" name="iniva" value="' + item.inIva + '"></td>\n' +
                                '<td><input type="hidden" value="' + item.idDetCompra + '"> ' + item.producto + textInva + ' </td>\n' +
                                '<td><input type="hidden" value="0">' + item.cantidad + '</td>\n' +
                                '<td><input type="hidden" name="unidad" value="' + item.unidadId + '">' + item.undText + '</td>\n' +
                                '<td><input type="hidden" value="0">' + item.costoUnidad + '</td>\n' +
                                '<td><input type="hidden" value="0">' + item.costoUnidad + '</td>\n' +
                                '<td><input type="hidden" value="">0.000</td>\n' +
                                '<td><input type="hidden" value="">0.0000</td>\n' +
                                '<td class="subtotal">' + item.odcostotot + '</td>\n' +
                        ...
                        $.when($("#table-productos-ventas tbody").html(tr)).done(loadTotalTd())
                    }
                })
            }

            $(document).on('click', '#btn-AnulacionFactura', function () {
                $("#modal-Anulacion").modal("show")
            })

            function pviewModal() {
                $("#pModal").modal('toggle').on("shown.bs.modal", function () {
                    $('#pModal .dataTables_filter input').focus();
                })
                $("#pModal .modal-header").css("background-color", "#84AC3B")
            }
            ...
            // devulve dinamicamente el numero de decimales que acepta la ventana de ingreso para los procesos de compras
            ...
            $("#ptable").DataTable({
                ...
                "ajax": {
                    "method": "POST",
                    "url": "./?action=processFactIngresos",
                    "data": {"option": 1}
                },
                "columns": [
                    {"data": "ruc"},
            ...
            $(document).on('click', '#ptable tbody tr td', function () { /* ==== Click sobre tbody CLIENTES ===== */
            ...
            $(document).on('click', '#anular', function () {
                let numero = $("#numeroFactura").val()
                let proveedor = $("#pcod").val()

                Swal.fire({
                    title: 'Anulación Factura?',
                    text: "Desea anular el Documento " + numero + "!",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: './?action=processAnularFact',
                            type: 'POST',
                            data: {'numero': numero, "proveedor": proveedor},
                            success: function (resp) {
                                let rest = JSON.parse(resp)
                                if (rest.substr(0, 1) == 0) {
                                    let r = new mensajes(rest.substr(2))
                                    r.error()
                                } else {
                                    let r = new mensajes(rest.substr(2))
                                    r.confirmed()
            ...
            /** */
            $(document).on('click', '#cargarIngresos', function () {
                let cliente = $("#cliente-cod").val()
                let sucursal = $("#sucursalid").val()
                if (cliente != '') {
                    loadIngresos(cliente, sucursal)
                } else {
                    Swal.fire({
                        icon: 'error',
            ...
            function loadIngresos(cliente, sucursal) {
                ...
                $("#table-view-egresos").DataTable({
                    ...
                    "ajax": {
                        "method": "POST",
                        "url": "./?action=processFactIngresos",
                        "data": {"option": opcion, "fecha": fecha, "cliente": cliente, "sucursal": sucursal}
                    },
            ...
            $(document).on('click', '#cargaXml', function () {
                $("#modal-archivo-xml").modal('show')
            })

            $(document).on("click", "#subir-archivo-xml", function (e) {
                e.preventDefault()
                $.ajax({
                    url: './?action=processFactIngresos',
                    type: 'POST',
                    data: new FormData(document.getElementById("form-archivo-xml")), // Enviar formulario con archivo por medio de ajax
                    contentType: false,
                    cache: false,
                    processData: false,
                    success: function (e) {
                        ...
                        let r = JSON.parse(e)
                        if (r.error == "") {
                            $("#ptoemision").val(r.emision)
                            $("#numerofact").val(r.secuencial[0])
                            $("#autorizacion").val(r.clave[0])
                            $("#cliente-cod").val(r.codigo[0])
                            $("#cliente-text").val(r.razon[0])

                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: r.error.substr(2),
                            }).then(() => {
            ...
            $(document).on('click', '#btn-cargarEgresos', function () {
                let proveedor = $("#cliente-cod").val()
                // console.log("facturacionproveedores")
                $("#table-view-egresos tbody tr").each(function (i, item) {
                    ...
                    if (r > 0) {
                        loadDetalle(ids, proveedor)
                        $("#modalEgresos").modal('hide')
                    }
                })
            })

            // funcion para cargar los detalles del ingreso para el proceso de facturacion correspondiente.
            function loadDetalle(ids, proveedor) {
                let opcion = 4
                $.ajax({
                    type: "POST",
                    url: "./?action=processFactIngresos",
                    data: {"option": opcion, "ids": ids, "proveedor": proveedor},
                    success: function (res) {
            ...
            // funcion para la edicion del precion dentro de la tabla del detalle de la factura.
            ...
            // funcion complementaria para el proceso de actualizar el precio dentro del detalle de la factura
            ...
            } // function pruebas de obtencion de datos

            /**/
            $('[data-toggle="tooltip"]').tooltip()

            // SELECCION DE TIPO DE COMPRA Q SE REALIZA -> PARA VENTAS CONTADO O CREDITO
            ...
            // VALIDA EL TIPO DE COMPRA QUE SE ESTA REALIZANDO CREDITO O CONTADO
...
// console.log("credito")
            ...
            xhttp.open("GET", './?action=getProductos&option=1')
            xhttp.send()

            $(document).on("keyup", "#buscar-productos", function () {
            ...
            $(document).on("click", "#btn-comprar", function (e) {
                ...
                if (validaNumProducts() == 0) {
                    ...
                    if (validaDocumentoAsociado().substr(0,1) == 1) {
                        console.log(validaDocumentoAsociado())
                        let msj = validaDocumentoAsociado()
                        Swal.fire({
            ...
            function validaDocumentoAsociado() {
                ...
                let r = ''
                $.ajax({
                    url: './?action=validaDocumentoAsociado',
                    type: 'POST',
                    async: false,
                    data: {"proveedor": proveedor, "factura": factura, "emision": emision},
                    success: function (datos) {
                        r = datos
                    }
                })
            ...
            function procesarCompra() {
                ...
                if (validaDataForFacturacion(cliente) === "") {
                    ...
                    *               ITERACION DE FORMA DE PAGO */
                    ...
                    formData.append('autorizacion', $("#autorizacion").val())
                    ...
                    formData.append('numerofact', $("#numerofact").val())
                    ...
                    $.ajax({
                        url: './?action=processFacturaCompra',
                        method: "POST",
                        data: formData,
                        ...
                        success: function (result) {
                            ...
                            if (datos.msj.substr(0, 1) == 0) {
                                ...
                                if (datos.emiteRet == 1) {
                                    proccessRetencion(datos.id) // procesa retencion de acuerdo a parametero de configuraciones generales
                                }

                                $("#numerofact").val(datos.documento.substr(6))
                                $("#btn-comprar").prop('disabled', true)

                                let link = "reportes/inventario/rideFacturaIng.php?id=" + datos.id
                                ...
                                /*if (datos.tipoEmision == 1) {
                                    ...
                                    let link = "reportes/inventario/rideFactura.php?id=" + datos.id
                                    $("#impresion-btn").attr('href', link)
                                    $("#impresion-btn").attr('target', "_blank")
                                }*/
                            }
            ...
            function proccessRetencion(idFiles) {
                $.ajax({
                    type: "POST",
                    url: './?action=processRetencion',
                    data: {option: 1, "documento": idFiles},
                    beforeSend: function () {
                        $("#myModalLoadingRennvio").modal('toggle')
                        $("#loading").addClass('loading');
                    },
                    success: function (data) {
                        ...
                        let msjMail = ''
                        if (r.msj !== null) {
                            if (r.msj[1].msjMail[0].substr(0, 1) == 1) {
                                msjMail = '<h4>' + r.msj[1].msjMail[0].substr(2) + '</h4>';
                            }
                        }
                        if (r.error == '') {
                            Swal.fire({
                                position: 'center',
                                icon: 'success',
                                html: '<h4>' + r.msj[0].msjAuto.substr(2) + '</h4>' + msjMail,
                                showConfirmButton: true,
                            })
                            let link = "reportes/generales/" + r.reporte + ".php?id=" + r.msj[2].id
                            $("#impresion-retencion-btn").attr('href', link)
                            $("#impresion-retencion-btn").attr('target', "_blank")
                            $("#myModalLoadingRennvio").modal('toggle')
                        } else {
                            Swal.fire({
                                position: 'center',
                                icon: 'error',
                                html: '<h4>' + r.error.substr(2) + '</h4>',
                                showConfirmButton: true,
                            }).then((result) => {
                                /* Read more about isConfirmed, isDenied below */
                                if (result.isConfirmed) {
...
// calSubtDesTotals()
            ...
            $(document).on('click', '#btnProveedor', function (e) {
                viewModalProveedores()
            })
            ...
            function loadProductForBodega() {
                ...
                xhttpLimit.open("GET", './?action=getDataCompras&option=15&bodega=')
                xhttpLimit.send()
            }
            ...
            * */
            function viewProducts() {
                ...
                if (codigos.length == 0) {
                    $.ajax({
                        type: "POST",
                        url: './?action=getDataCompras',
                        data: {option: 1, bodega: bodega},
                        beforeSend: function () {
                        },
                        error: function () {
                        },
                    ...
                    $.ajax({
                        type: "POST",
                        url: './?action=getDataCompras',
                        data: {"option": 16, "lastprod": countFilasTable(), "bodega": bodega},
                        // dataType: "html",
                        beforeSend: function () {
                        },
                        error: function () {
                        ...
                        success: function (data) {
                            console.log(data + "tiene mas de un producto agregado")
                            let datos = JSON.parse(data)
                            let tableBodyProduct = ''
                            let t = 1
                            let top = 10
            ...
            }  /** ACTUALIZACION DE CAMPOS POR ACTUALIZACION DE PAGINA PARA COMPRAS */
            ...
            * */
            ...
            function viewModalProveedores() {
                $("#modalProveedores").modal('toggle').on("shown.bs.modal", function () {
                    $('#modalProveedores .dataTables_filter input').focus();
                })
                $("#modalProveedores .modal-header").css("background-color", "#84AC3B")
            ...
            $(document).on('click', '#tbody-proveedor tr td', function () {
                let name = $(this).parents("tr").find("td").eq(1).text()
                let ruc = $(this).parents("tr").find("td").eq(0).text()
                $("#cliente-cod").val(ruc)
                $("#cliente-text").val(name)
                $("#modalProveedores").modal('hide')
                // validaFucClient(ruc) // esta funcion valida clientes
                $.post("./?action=getDataCompras", {ceruc: ruc, option: 2}, function (result) {
                    let plazo = JSON.parse(result)
                    $("#plazo").val(plazo)
                });

            }) /* ======= SELECCIONA EL PROVEEDOR Y LO CARGA EN LA SECCION DE PROVEEDOR DE LA VENTANA DE COMPRAS  ======= */ // ACTUALIZACION CORRESPONDIENTE A LA VENTANA DE COMPRA
...
// $("#table-tbody-products").html('<tr><td colspan="2">Sin resultados....</td</tr>');
            ...
            function loadLineaProduct() {
                ...
                if (producto != '') {
                    ...
                    if (document.getElementById('desc1') && document.getElementById('desc1').value != "") {
                        desc1 = $("#desc1").val()
                    ...
                    if (document.getElementById('desc2') && document.getElementById('desc2').value != "") {
                        desc2 = $("#desc2").val()
                    ...
                    let nw_precio = precio - (valDesc1 + valDesc2).toFixed(calDecimalesVentas())
                    ...
                    option += '<td>' + codigo + '<input type="hidden" name="iniva" value="' + iniva + '"></td>' +
                        ...
                        '<td><input type="hidden" value="' + funcion + '">' + cantidad + '</td>' +
                        '<td><input type="hidden" name="unidad" value="' + unidtipo + '">' + unidtext + '</td>' +
                        '<td><input type="hidden" value="' + ccosto + '">' + precio + '</td>' +
                        '<td><input type="hidden" value="' + unidad + '">' + (cantidad * nw_precio).toFixed(2) + '</td>' +
                        '<td style="display: none"><input type="hidden" value="' + desc1 + '" >' + valDesc1.toFixed(calDecimalesVentas()) + '</td>' + // subtotal
                        '<td style="display: none"><input type="hidden" value="' + desc2 + '" >' + valDesc2.toFixed(calDecimalesVentas()) + '</td>' +
                        '<td class="subtotal">' + total.toFixed(2) + '</td>' +
                        '<td><buttom class="btn btn-deldoc btn-xs btn-removeRow"><i class="glyphicon glyphicon-minus"></i></buttom></td>' +
                        '<td style="display: none">' + com1 + '</td>' +
                        '<td style="display: none">' + com2 + '</td>' +
                        '<td style="display: none">' + com3 + '</td>'
                    option += '</tr>'
            ...
            if (document.getElementById('desc1')) {
                $('#desc1').blur(function () {
                    ...
                    $.ajax({
                        url: './?action=getDataCompras',
                        type: 'post',
                        data: {option: 13, desc1: valor, tdes: 1},
                        success: function (resp) {
            ...
            if (document.getElementById('desc2')) {
                $('#desc2').blur(function () {
                    ...
                    $.ajax({
                        url: './?action=getDataCompras',
                        type: 'post',
                        data: {option: 13, desc2: valor, tdes: 2},
                        success: function (resp) {
                            // console.log(resp)
                            let r = JSON.parse(resp)
                            let modal = 0
            ...
            * */
            ...
            function loadPedidosForBodega(bodega) {
                $.post("./?action=getDataCompras", {bodega: bodega, option: 14})
                    .done(function (data) {
