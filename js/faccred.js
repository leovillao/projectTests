if (document.getElementById('factCRE')) {
    $(document).ready(function () {
            ...
            $(document).on("keydown", ".checkDecimales", function (r) {
                ...
                let regex = new RegExp("^\\d{0,9}(\\.\\d{1," + calDecimalesVentas() + "})?$")
                setTimeout(function () {
            ...
            function procesarCompra() {
                ...
                if (validaDataForFacturacion(cliente) === "") {
                    ...
                    $.ajax({
                        url: './?action=processFacturacionCred',
                        method: "POST",
                        data: formData,
                        ...
                        success: function (result) {
                            ...
                            if (datos.msj.substr(0, 1) == 0) {
                                ...
                                $("#btn-comprar").prop('disabled', true)
                                let link = "reportes/generales/" + datos.reporte + ".php?id=" + datos.id
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (dat[0]['msjAuto'].substr(0, 1) == 1) {
                            ...
                            // window.open("reportes/generales/"+data[0]['reporte']+".php?id=" + dat[2]['id'], '_blank');
            ...
            * */
            ...
            // FUNCION PARA VALIDAR SI PRODUCTO SE MOSTRARA CON IVA O SIN IVA DE ACUERDO A PUNTO DE EMISION

            function validaEmisionIva() {
                let puntoEmision = $("#ptoemision").val()
                let r = 0
                $.ajax({
                    async: false,
                    url: './?action=validaImpuestoProduct',
                    type: 'POST',
                    data: {
                        // "itcodigo": itcodigo,
                        "emision": puntoEmision
                    },
            ...
            // FUNCION PARA SELECCIONAR EL PRODUCTO DESDE LA TABLA DE PRODUCTO BUSQUEDA
            ...
            function selectProducto(row, marcador) {
                ...
                if (validaEmisionIva() == 1) { // VALIDA SI EL PUNTO DE EMISION GRABA IMPUESTO
                    $("#iniva").val(0)
                } else {
                    $("#iniva").val(iniva)
                }
                ...
                if (unidad != '') {
                    ...
                    $.ajax({
                        ...
                        success: function (e) {
                            ...
                            if (dat.lista === true) {
                                let t = dat.precio.precio * factor
                                $("#precion").val(t.toFixed(calDecimalesVentas()))
                                let nprice = dat.precio.precio * cantidad
                                $("#total").val(Number(nprice).toFixed(calDecimalesVentas()))
            ...
            function loadTotal() {
                ...
                $("#total").val(total.toFixed(calDecimalesVentas()))
            } /* FUNCION QUE CALCULA EL TOTAL DEL PRECIO POR LA CANTIDAD QUE SE VA A INGRESAR AL FORMULARIO DE COMPRA */


            function validaPrecioPorCosto(precio, producto) {
                let r = ''
                $.ajax({
                    url: "./?action=factCred_validaPrecioCosto",
                    type: "POST",
                    async: false,
            ...
            $(document).on('click', '#addLineaProducto', function (e) {
                ...
                let precio = $("#precion").val()
                let producto = $("#producto-cod").val()
                if (validaPrecioPorCosto(precio, producto) == "") {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: validaPrecioPorCosto(precio, producto),
                    })
                }
                bloqueFecha()
            ...
            function loadLineaProduct() {
                ...
                if (producto != '') {
                    ...
                    option += '<tr>'
                    option += '<td>' + codigo + '<input type="hidden" name="iniva" value="' + iniva + '"></td><td>' + descripcion + txtStock + txtIva + '</td><td><input type="hidden" value="' + funcion + '">' + cantidad + '</td><td><input type="hidden" name="unidad" value="' + unidtipo + '">' + unidtext + '</td><td><input type="hidden" value="' + ccosto + '">' + precio + '</td><td><input type="hidden" value="' + desc1 + '" >' + valDesc1.toFixed(calDecimalesVentas()) + '</td><td><input type="hidden" value="' + desc2 + '" >' + valDesc2.toFixed(calDecimalesVentas()) + '</td><td><input type="hidden" value="' + unidad + '">' + nw_precio.toFixed(calDecimalesVentas()) + '</td><td class="subtotal">' + total.toFixed(calDecimalesVentas()) + '</td><td><buttom class="btn btn-deldoc btn-xs btn-removeRow"><i class="glyphicon glyphicon-minus"></i></buttom></td><td style="display: none">' + com1 + '</td><td style="display: none">' + com2 + '</td><td style="display: none">' + com3 + '</td>'
                    option += '</tr>'
                    ...
                    $.ajax({
                        ...
                        success: function (precio) {
                            ...
                            if (d != '') {
                                ...
                                row.find('td').eq(8).text(total.toFixed(calDecimalesVentas())) // Nuevo precio para el producto de la tabla de venta
                            }
                        }
                    })
                })
            ...
            function loadTotalTd() {
                ...
                let el = $("#ptoemision")
                let impuesto = $('option:selected', el).attr('impuesto');

                $("#tbody-saleProducts tr").each(function () {
                    let row = $(this)
                    let iniva = row.find('td').eq(0).find('input[type="hidden"]').val()
                    if (iniva == 1 && impuesto == 0) {
                ...
                $("#bgravada").val(valoriva.toFixed(calDecimalesVentas()))
                $("#bexenta").val(valorsiva.toFixed(calDecimalesVentas()))
                $("#ventab").val(valor.toFixed(calDecimalesVentas()))
                desvalor = $("#dvalor").val()
                subtotal = valor - desvalor
                $("#subtotal").val(subtotal.toFixed(calDecimalesVentas()))
                ivaval = $("#ivaval").val()
                iva = valoriva * ivaval / 100
                $("#iva").val(iva.toFixed(calDecimalesVentas()))
                total = subtotal + iva
                $("#totalpago").val(total.toFixed(calDecimalesVentas()))
                $("#totaFormapago").val(total.toFixed(calDecimalesVentas()))
            ...
            function calcularDescuento1() {
                let pvp = $("#precion").val()
                let dsc1 = $("#desc1").val()
                let pvpDes1 = pvp - parseFloat(pvp * dsc1 / 100).toFixed(calDecimalesVentas())
                let cantidad = $("#cantidad").val()
                let valTotal = cantidad * pvpDes1
                $("#total").val(valTotal.toFixed(calDecimalesVentas()))
            ...
            function calcularDescuento2() {
                if (document.getElementById('desc1')) {
                    ...
                    let pvpDes1 = pvp - parseFloat(pvp * dsc1 / 100).toFixed(calDecimalesVentas())
                    let dsc2 = $("#desc2").val()
                    let pvpDes2 = pvpDes1 - parseFloat(pvpDes1 * dsc2 / 100).toFixed(calDecimalesVentas())
                    let cantidad = $("#cantidad").val()
                    ...
                    let pvp = $("#precion").val()
                    let dsc2 = $("#desc2").val()
                    let pvpDes2 = pvp - parseFloat(pvp * dsc2 / 100).toFixed(calDecimalesVentas())
                    let cantidad = $("#cantidad").val()
                    let valTotal = cantidad * pvpDes1
                    $("#total").val(valTotal.toFixed(calDecimalesVentas()))
                }
                ...
                if (entidadid == 0) {
                    ...
                    if (valorFP != 0) {
                        ...
                        if (parseFloat(valorFP) > parseFloat(valorPG)) {
                            ...
                            $("#cambio").text("$ " + cambio.toFixed(calDecimalesVentas()))
                            $("#totaFormapago").val(0)
                            ...
                            $("#tableBody-fp").append(htmlpagos)
                            $("#cambio").text("$ 0.00")
                            $("#totaFormapago").val(saldo.toFixed(calDecimalesVentas()))
                            // $("#totaFormapago").attr('readOnly', 'readOnly')
                        }
                    }
                }
            ...
            function calDesPorcentaje(porcentaje) {
                if (parseInt(porcentaje) == 0 || porcentaje == '') {
                    ...
                    $("#dvalor").val(valorDescuento.toFixed(calDecimalesVentas()))
                    let nsubt = ventb - valorDescuento // subtotal menos descuento
                    $("#subtotal").val(nsubt.toFixed(calDecimalesVentas()))
                    let ngrab = grabado - (grabado * porcentaje / 100)
                    $("#bgravada").val(ngrab.toFixed(calDecimalesVentas()))
                    let nexe = exento - (exento * porcentaje / 100)
                    $("#bexenta").val(nexe.toFixed(calDecimalesVentas()))
                    let niva = ngrab * ivapor / 100
                    $("#iva").val(niva.toFixed(calDecimalesVentas()))
                    let ntotal = ngrab + nexe + niva
                    $("#totalpago").val(ntotal.toFixed(calDecimalesVentas()))
                    $("#totaFormapago").val(ntotal.toFixed(calDecimalesVentas()))
                }
            ...
            function calPorDescuento(porcentaje) {
                if (parseInt(porcentaje) == 0 || porcentaje == '') {
                    ...
                    $("#val_des").val(val_des.toFixed(calDecimalesVentas()))
                    $("#dporcen").val(descuentoPorcentaje.toFixed(calDecimalesVentas()))
                    let grabado = $("#bgravada").val()
                    let exento = $("#bexenta").val()
                    ...
                    $("#subtotal").val(subtotal.toFixed(calDecimalesVentas()))
                    $("#bgravada").val(desGrab.toFixed(calDecimalesVentas()))
                    $("#bexenta").val(desExe.toFixed(calDecimalesVentas()))
                ...
                $.ajax({
                    ...
                    success: function (e) {
                        ...
                        $.each(s.datos, function (i, item) {
                            option += '<option value="' + item.id + '"  impuesto="' + item.impuesto + '" >' + item.estab + '-' + item.emision + '</option>'
                        })
                ...
                $.ajax({
                    ...
                    success: function (e) {
                        ...
                        $.each(s.emisiones, function (i, item) {
                            option += "<option value='" + item.id + "' " + item.selected + " impuesto='" + item.impuesto + "' >" + item.emision + "</option>"
                        })
            ...
            $(document).on("change", "#ptoemision", function () {
                let valor = $('option:selected', $(this)).attr("impuesto")
                cargaInputGrabado(valor)
            })

            function cargaInputGrabado(valor) {
                if (valor == 1) {
                    $(".groupImpuestos").css('display', '')
                    $(".groupImpuestos").addClass('camposGroupImpuestos')
                } else {
                    $(".groupImpuestos").removeClass('camposGroupImpuestos')
                    $(".groupImpuestos").css('display', '')
                    $(".groupImpuestos").css('display', 'flex')
                }
            }
            ...
            function loadPedidosData(numPedido) {
                ...
                $.post("./?action=loadPedforFact", {pedido: numPedido})
                    .done(function (data) {
                        ...
                        $("#bgravada").val(parseFloat(tiva).toFixed(calDecimalesVentas()))
                        $("#bexenta").val(parseFloat(tsiva).toFixed(calDecimalesVentas()))
                        $("#ventab").val(valor.toFixed(calDecimalesVentas()))
                        desvalor = $("#dvalor").val()
                        subtotal = valor - desvalor
                        $("#subtotal").val(parseFloat(subtotal).toFixed(calDecimalesVentas()))
                        ivaval = $("#ivaval").val()
                        iva = tiva * parseInt(ivaval) / 100
                        $("#iva").val(parseFloat(iva).toFixed(calDecimalesVentas()))
                        totalt = subtotal + iva
                        $("#totalpago").val(parseFloat(totalt).toFixed(calDecimalesVentas()))
                        $("#totaFormapago").val(parseFloat(totalt).toFixed(calDecimalesVentas()))
