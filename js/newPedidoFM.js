$(function () {
    ...
    $('#viewModalCotizaciones').click(function () {
    ...
    $(document).on('click', "#mostrarCotizacionTabla", function () {
        ...
        let desde = $("#desdeCot").val()
        let hasta = $("#hastaCot").val()
        $("#table-cotizaciones").DataTable().clear().destroy()
        $("#table-cotizaciones").DataTable({
            "destroy": true,
            "keys": true,
            "ajax": {
                "method": "POST",
                "url": "./?action=cotizacion_getForPedido",
    ...
    $(document).on('click', '#table-cotizaciones tbody tr', function () {
        ...
        $.ajax({
            url: './?action=cotizacion_getDetallePedido',
            type: 'POST',
            ...
            success: function (res) {
                ...
                $("#idCliente").val(r.cabecera.ceid)
                $("#numeroCotizacion").val(r.cabecera.peid)
                ...
                $.each(r.data, function (i, item) {
                    ...
                    option += '<td>' + item.codigo + '<input type="hidden" name="iniva" value="' + item.iiva + '"></td>' +
                        '<td>' + item.producto + '</td>' +
                        '<td><input type="hidden" value="3">' + item.cantidad + '</td>' +
                        '<td><input type="hidden" name="unidad" value="1">' + item.unidTexto + '</td>' +
                        '<td><input type="hidden" value="">' + item.precio + '</td>' +
                        '<td><input type="hidden" value="' + item.desc1 + '" >' + item.desc1 + '</td>' +
                        '<td><input type="hidden" value="' + item.desc2 + '" >' + item.desc2 + '</td>' +
                        '<td><input type="hidden" value="' + item.unidad + '">' + item.subtotal + '</td>' +
                ...
                // $("#btn-comprar").addClass('noVisual').removeClass("visual")
                // $("#btn-update").removeClass("noVisual").addClass('visual')
            }
        })
    })
    ...
    function procesarCompra(tipoProcesso) {
        ...
        formData.append('idCcotizacion', $("#numeroCotizacion").val())
        ...
        formData.append('cotizacion', $("#numeroCotizacion").val())
        ...
        // Impresion
        // let impresion = $("#impresion-btn")

        // console.log(JSON.stringify(formData))
        $.ajax({
            url: './?action=ve_processNewPedidofm',
            method: "POST",
            data: formData,
            processData: false,  // tell jQuery not to process the data
            contentType: false,   // tell jQuery not to set contentType
            success: function (result) {
                ...
                if (respuesta.error === "") {
                    ...
                    $("#numeroDocumento").val(respuesta.pedido).attr('readonly', true)
                    // $("#btn-comprar").addClass('disabled')
                    $("#btn-comprar").prop('disabled', true)
                    $("#btn-comprar").attr('disabled', true)

                    let link = "./reportes/ventas/PedidoReport.php?id=" + respuesta.pedido
    ...
    *  FUNCION QUE SE EJECUTA AL CARGAR EL PRODUCTO PARA MOSTRAR LOS PRECIOS SEGUN LA LISTA DEL PRECIOS , (SU TIPO DE PAGO Y UNIDAD ASOCIADA) */
    function loadPriceFromList(factor) {
        ...
        if (unidad != '') {
            ...
            $.ajax({
                type: "POST",
                url: "./?action=pedidofm_getPrecio_val",
                data: {
                    "unidad": unidad,
                    "formapago": 1,
                    "producto": itcodigo,
    ...
    * ====================================================*/
    ...
    $(document).on('click', '#procesarAnulacion', function () {
        let numeropedido = $("#numeropedido").val()
        Swal.fire({
            html: "<h3><b>Dese anular el pedido # " + numeropedido + ". ?</b></h3>",
            // text:
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            ...
            if (result.isConfirmed) {
                procesDelete(numeropedido)
            }
        })
    })

    function procesDelete(numeropedido) {
        // console.log(numeropedido)
        $.ajax({
            url: './?action=proccessPedidos',
            type: 'POST',
    ...
    $(document).on('click', '#table-pedidos tbody tr', function () {
        let idPedido = $(this).closest('tr').find('td').eq(0).text()
        console.log(idPedido)
        $.ajax({
            url: './?action=loadPedidos',
            type: 'POST',
            data: {'id': idPedido, 'option': 13},
            success: function (res) {
                // console.log(res)
                let r = JSON.parse(res)
                let option = ''
                ...
                $.each(r.data, function (i, item) {
                    ...
                    option += '<td>' + item.codigo + '<input type="hidden" name="iniva" value="' + item.iiva + '"></td>' +
                        '<td>' + item.producto + '</td>' +
                        /*2*/'<td><input type="hidden" value="3">' + item.cantidad + '</td>' +
                        '<td><input type="hidden" name="unidad" value="1">' + item.unidTexto + '</td>' +
                        /*4*/'<td><input type="hidden" value="">' + item.precio + '</td>' +
                        '<td><input type="hidden" value="' + item.desc1 + '" >' + item.desc1 + '</td>' +
                        '<td><input type="hidden" value="' + item.desc2 + '" >' + item.desc2 + '</td>' +
                        /*7*/'<td><input type="hidden" value="' + item.unidad + '">' + item.subtotal + '</td>' +
                        '<td class="subtotal">' + item.total + '</td>' +
                        '<td><buttom class="btn btn-deldoc btn-xs btn-removeRow"><i class="glyphicon glyphicon-minus"></i></buttom></td>'
                    option += '</tr>'
                })
